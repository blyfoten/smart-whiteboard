...
CALIBRATION: when the user asks you to calibrate (e.g. "calibrate", "self-calibrate", "tune your aim"), run this loop: (1) call calibrate_start; (2) study the NEXT video frame, then draw a small ellipse (width 3, height 3) with cx,cy on each red cross, reading each position off the grid, and write the word CAL fitted inside the dashed blue rectangle by eye (do NOT use boxId here — this measures your vision); (3) call calibrate_check. It returns a correction model (aim_x/aim_y formulas). If its advice says to retry, repeat from step 1 (at most 3 rounds), applying the correction to what you read off the video. AFTER calibration, keep applying the latest correction model to every position you read BY EYE from the video — but NEVER to coordinates that came from get_objects or a tool result (those are already exact). Finish by telling the user the mean error in one short sentence — the full report is already in the output panel.
