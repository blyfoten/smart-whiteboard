...
const TOOLS = Type ? [{
    functionDeclarations: [
        ...
        { name: 'draw_rect', description: 'Draw a rectangle of size (width,height) percent. Position it EITHER by center (cx,cy — preferred when centering on/around something) OR by top-left corner (x,y). Optional: color (outline), strokeWidth (px), fill (CSS name/hex; omit for transparent), fillOpacity (0-1), cornerRadius (px, for rounded corners).', parameters: { type: Type.OBJECT, properties: { x: { type: Type.NUMBER }, y: { type: Type.NUMBER }, cx: { type: Type.NUMBER }, cy: { type: Type.NUMBER }, width: { type: Type.NUMBER }, height: { type: Type.NUMBER }, color: { type: Type.STRING }, strokeWidth: { type: Type.NUMBER }, fill: { type: Type.STRING }, fillOpacity: { type: Type.NUMBER }, cornerRadius: { type: Type.NUMBER } }, required: ['width', 'height'] } },
        { name: 'draw_ellipse', description: 'Draw an ellipse/circle of size (width,height) percent. Position it EITHER by center (cx,cy — preferred; e.g. a circle AT a point) OR by the bounding box top-left (x,y). Optional: color (outline), strokeWidth (px), fill (CSS name/hex; omit for transparent), fillOpacity (0-1).', parameters: { type: Type.OBJECT, properties: { x: { type: Type.NUMBER }, y: { type: Type.NUMBER }, cx: { type: Type.NUMBER }, cy: { type: Type.NUMBER }, width: { type: Type.NUMBER }, height: { type: Type.NUMBER }, color: { type: Type.STRING }, strokeWidth: { type: Type.NUMBER }, fill: { type: Type.STRING }, fillOpacity: { type: Type.NUMBER } }, required: ['width', 'height'] } },
        ...
        { name: 'calibrate_check', description: 'Finish a calibration round: measures how far your marks landed from the targets, prints a report to the output panel, cleans the board, and returns the error stats plus advice on whether to retry. Pass keep=true to leave the targets and your marks visible on the board (e.g. when the user wants a screenshot); they are swept by the next calibrate_start.', parameters: { type: Type.OBJECT, properties: { keep: { type: Type.BOOLEAN } } } },
...
CALIBRATION: when the user asks you to calibrate (e.g. "calibrate", "self-calibrate", "tune your aim"), run this loop: (1) call calibrate_start; (2) study the NEXT video frame, then draw a small ellipse (width 3, height 3) with cx,cy on each red cross, reading each position off the grid, and write the word CAL with cx,cy on the CENTER of the dashed blue rectangle, sized ~70% of the box height (do NOT use boxId here — this measures your vision); (3) call calibrate_check. On the grid, strong NUMBERED lines are the 10s (labels on all four edges) and thin faint lines are the 5s (15, 25, 35...); a target often sits ON a 5-line or between lines, so read every coordinate to the nearest 1 — never snap to the nearest numbered line. calibrate_check returns a correction model (aim_x/aim_y formulas) and flags "slips" (marks that snapped to a wrong gridline). If its advice says to retry, repeat from step 1 (at most 3 rounds), following the advice. AFTER calibration, keep applying the latest correction model to every position you read BY EYE from the video — but NEVER to coordinates that came from get_objects or a tool result (those are already exact). A SYSTEM NOTE at the start of a session may carry a stored correction model from an earlier calibration — apply it the same way without mentioning it. If the user wants to SEE or screenshot the result, pass keep=true to calibrate_check so the marks stay on the board. Finish by telling the user the mean error in one short sentence — the full report is already in the output panel.
