...
const TOOLS = Type ? [{
    functionDeclarations: [
        ...
        { name: 'get_objects', description: 'List everything on the board with EXACT positions and sizes in percent (0-100): index, type, x, y, width, height. Call this BEFORE drawing anything that must align with, match, or be placed relative to an existing shape — use the returned numbers instead of estimating from the image.', parameters: { type: Type.OBJECT, properties: {} } },
...
IMPORTANT for accuracy: your eyeball estimates from the image are imprecise. Whenever a drawing must align with, match, or be positioned relative to something already on the board, FIRST call get_objects to read the exact coordinates, then draw using those numbers. To replicate a shape the user drew, prefer duplicate_object (a pixel-exact copy) over re-drawing it.
