...
const TOOLS = Type ? [{
    functionDeclarations: [
        { name: 'draw_line', description: 'Draw a straight line. x1,y1,x2,y2 are percentages 0-100 of the board (origin top-left). Optional color (CSS name or hex) and strokeWidth (px).', parameters: { type: Type.OBJECT, properties: { x1: { type: Type.NUMBER }, y1: { type: Type.NUMBER }, x2: { type: Type.NUMBER }, y2: { type: Type.NUMBER }, color: { type: Type.STRING }, strokeWidth: { type: Type.NUMBER }, fromVideo: { type: Type.BOOLEAN } }, required: ['x1', 'y1', 'x2', 'y2'] } },
        { name: 'draw_rect', description: 'Draw a rectangle of size (width,height) percent. Position it EITHER by center (cx,cy — preferred when centering on/around something) OR by top-left corner (x,y). Optional: color (outline), strokeWidth (px), fill (CSS name/hex; omit for transparent), fillOpacity (0-1), cornerRadius (px, for rounded corners).', parameters: { type: Type.OBJECT, properties: { x: { type: Type.NUMBER }, y: { type: Type.NUMBER }, cx: { type: Type.NUMBER }, cy: { type: Type.NUMBER }, width: { type: Type.NUMBER }, height: { type: Type.NUMBER }, color: { type: Type.STRING }, strokeWidth: { type: Type.NUMBER }, fill: { type: Type.STRING }, fillOpacity: { type: Type.NUMBER }, cornerRadius: { type: Type.NUMBER }, fromVideo: { type: Type.BOOLEAN } }, required: ['width', 'height'] } },
        { name: 'draw_ellipse', description: 'Draw an ellipse/circle of size (width,height) percent. Position it EITHER by center (cx,cy — preferred; e.g. a circle AT a point) OR by the bounding box top-left (x,y). Optional: color (outline), strokeWidth (px), fill (CSS name/hex; omit for transparent), fillOpacity (0-1).', parameters: { type: Type.OBJECT, properties: { x: { type: Type.NUMBER }, y: { type: Type.NUMBER }, cx: { type: Type.NUMBER }, cy: { type: Type.NUMBER }, width: { type: Type.NUMBER }, height: { type: Type.NUMBER }, color: { type: Type.STRING }, strokeWidth: { type: Type.NUMBER }, fill: { type: Type.STRING }, fillOpacity: { type: Type.NUMBER }, fromVideo: { type: Type.BOOLEAN } }, required: ['width', 'height'] } },
        { name: 'draw_arrow', description: 'Draw an arrow from (x1,y1) to (x2,y2), percent 0-100. Optional color and strokeWidth (px).', parameters: { type: Type.OBJECT, properties: { x1: { type: Type.NUMBER }, y1: { type: Type.NUMBER }, x2: { type: Type.NUMBER }, y2: { type: Type.NUMBER }, color: { type: Type.STRING }, strokeWidth: { type: Type.NUMBER }, fromVideo: { type: Type.BOOLEAN } }, required: ['x1', 'y1', 'x2', 'y2'] } },
        { name: 'draw_polyline', description: 'Draw a connected multi-segment line (or a closed polygon) through a list of points, each {x,y} in percent 0-100. Its vertices are editable. Set closed=true for a filled/closed polygon. Vertices placed near an existing shape\'s edge automatically anchor to it and follow that shape when it moves (pass anchor=false to disable). Optional: color (outline), strokeWidth (px), fill + fillOpacity (polygons only), closed, anchor.', parameters: { type: Type.OBJECT, properties: { points: { type: Type.ARRAY, items: { type: Type.OBJECT, properties: { x: { type: Type.NUMBER }, y: { type: Type.NUMBER } }, required: ['x', 'y'] } }, closed: { type: Type.BOOLEAN }, anchor: { type: Type.BOOLEAN }, color: { type: Type.STRING }, strokeWidth: { type: Type.NUMBER }, fill: { type: Type.STRING }, fillOpacity: { type: Type.NUMBER }, fromVideo: { type: Type.BOOLEAN } }, required: ['points'] } },
        { name: 'draw_polygon', description: 'Draw a closed polygon through a list of points, each {x,y} in percent 0-100 (a draw_polyline with closed=true). Vertices are editable and anchor to nearby shapes. Optional: color, strokeWidth, fill, fillOpacity, anchor.', parameters: { type: Type.OBJECT, properties: { points: { type: Type.ARRAY, items: { type: Type.OBJECT, properties: { x: { type: Type.NUMBER }, y: { type: Type.NUMBER } }, required: ['x', 'y'] } }, anchor: { type: Type.BOOLEAN }, color: { type: Type.STRING }, strokeWidth: { type: Type.NUMBER }, fill: { type: Type.STRING }, fillOpacity: { type: Type.NUMBER }, fromVideo: { type: Type.BOOLEAN } }, required: ['points'] } },
        { name: 'write_text', description: "Write text. Position it ONE of three ways: boxId (a shape's id — auto-centers and auto-sizes the text to fit inside that shape; ALWAYS prefer this for a label inside a box/ellipse), cx,cy percent (the text's CENTER — prefer this when centering on a point), or x,y percent (top-left). size is optional (percent of board height, default 6). Optional color (CSS name or hex).", parameters: { type: Type.OBJECT, properties: { x: { type: Type.NUMBER }, y: { type: Type.NUMBER }, cx: { type: Type.NUMBER }, cy: { type: Type.NUMBER }, text: { type: Type.STRING }, size: { type: Type.NUMBER }, color: { type: Type.STRING }, boxId: { type: Type.STRING }, fromVideo: { type: Type.BOOLEAN } }, required: ['text'] } },
