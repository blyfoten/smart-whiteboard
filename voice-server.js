...
const CANVAS_TOOLS = Type ? [
        ...
        { name: 'cad_offset', description: "CAD: parallel-copy an existing connected run of lines at a perpendicular distance (sketch units, signed — negative flips the side) — the fast way to build a wall/panel's second face instead of computing and drawing it by hand. entityIds must be ONE connected chain with no branches (get them from cad_sketch_line/polyline/rect or cad_get_sketch). Each new segment is constrained parallel to its source. If the chain is open, end-cap lines are added too, already dimensioned to the offset distance — that dimension is the wall's driving thickness. Returns lineIds/pointIds for the new chain and capLineIds for the caps (may be empty for a closed chain).", parameters: { type: Type.OBJECT, properties: { entityIds: { type: Type.ARRAY, items: { type: Type.STRING } }, distance: { type: Type.NUMBER } }, required: ['entityIds', 'distance'] } },
] : [];
...
- TECHNICAL DRAWING AT REAL SIZES ("make the walls 20 thick, add doors and windows"): use CAD. Draw the wall centreline with cad_sketch_line/cad_sketch_polyline, then cad_offset it by the thickness to get the other face in one call — no need to compute parallel endpoints by hand. Keep the thickness in a parameter (cad_set_param, e.g. name 'thickness' value '20') so the plan can be re-thicknessed later; cad_offset's distance itself must be a plain number (not an expression like cad_dimension takes), so read the parameter's current value and pass that number. An opening is a gap: draw the wall as segments that stop either side of the door or window, and add the door swing as an arc-like polyline or the window as a thin rectangle between the faces. Watch solve.ok and degreesOfFreedom as you go.
