...
`npm test` runs the pure-logic checks (plain Node, no framework): `test/shape-classifier.test.mjs` (stroke classifier) and `test/cad-core.test.mjs` (CAD expression evaluator, sketch model, constraint solver). There is no linter configured.
